...
module.exports = BaseCommand.extend({
    ...
    options: [{
        ...
        name: 'noconfirm',
        description: 'No user inputs, auto start app "yes/no"',
        defaultValue: 'yes'
    }],
    ...
    execute: function (version, options) {
        ...
        // Parameter url should be set if noconfirm is enable
        if (options.noconfirm && !options.url) {
            throw new Error('--url should be set if --noconfirm is enable.');
        }
        ...
        return findVersion(version).then(function runDoctor(resolvedVersion) {
            ...
            if (options.noconfirm) {
                if (options.noconfirm === 'yes') {
                    return Promise.resolve({start: true});
                }
                return Promise.resolve({start: false});
            }
